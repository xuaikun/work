...
int main()
{
	...
	for (int iNum1 = 0; iNum1 < imgNum1; iNum1++)
	{
		...
		for (int iNum2 = 0; iNum2 < imgNum2; iNum2++)
		{
			...
			StringToChar(imgname, imgname_str);
			StringToChar(imgname1, imgname1_str);
			...
			string strSum = "E:\\000007work\\Sample";
			char strSum_new[100] = {};
			...
			if ((Get_Score >= 55) && (Get_Score < 60))
			{
				...
				// 剪切文件到指定的文件夹
				// 文件剪切操作 成功
				MoveFileA((LPCSTR)imgNames[0][iNum1].c_str(), (LPCSTR)pathName);
		...
		// 这里表示从测试文件夹中抽出的照片不属于任何一个样本文件夹，只能把该图片剪切到laji文件夹
		{
			...
			string strFinal = "E:\\000007work";
			string strFinal_new = strFinal + "\\" + "laji";
